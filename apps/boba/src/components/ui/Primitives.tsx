...
import { formatScore, type Cup } from "@/lib/boba"
  ...
  return (
    <Link
      ...
      style={{
        ...
        height, overflow: "hidden", borderRadius: 18, padding: photo ? 0 : 8, textAlign: "center",
    ...
    >
      {photo ? (
        ...
        <>
          <span className="t-score" style={{ fontSize: 44, lineHeight: 1 }}>{formatScore(review.score)}</span>
          <span className="t-label" style={{ letterSpacing: "0.08em", overflowWrap: "anywhere" }}>{name}</span>
        </>
      )}
    </Link>
  )
