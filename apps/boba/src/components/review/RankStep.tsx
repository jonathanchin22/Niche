...
import { formatScore, isHomePlace, type Cup } from "@/lib/boba"
 ...
 * new sip into the person's own ranking (the Beli mechanic). Skippable; the
...
export default function RankStep({ userId, cup, ladder, onDone }: {
  ...
  const rivalName = rival.item_name ?? rival.category ?? "that sip"
  const rivalPlace = isHomePlace(rival.place) ? "made at home" : rival.place?.name ?? ""
...
function Choice({ label, name, place, score, photo, disabled, onPick }: {
  ...
  return (
    <button type="button" onClick={onPick} disabled={disabled} aria-label={`${label}: ${name}${place ? `, ${place}` : ""}`}
      ...
      {photo
        ? <img src={photo} alt="" className="photo" style={{ height: 210, borderRadius: 18 }} />
        : (
          <span className="t-display" style={{ height: 210, width: "100%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 64, background: "var(--c-paper)", border: "1px solid var(--c-rule)", borderRadius: 18 }}>
            {formatScore(score)}
          </span>
        )}
