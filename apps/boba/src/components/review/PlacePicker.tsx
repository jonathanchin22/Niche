...
/**
 * The "where" field: shops near you appear as soon as the screen opens, typing
...
export default function PlacePicker({ id, query, place, onQueryChange, onPick }: {
  ...
  // "You're at <shop>": auto-picked when the person is clearly inside one.
  ...
  useEffect(() => {
    ...
    getCurrentPosition().then(async pos => {
      ...
      setHere(pos)
      const found = await findNearbyPlaces("boba", pos, { limit: 30, signal: ctrl.signal }).catch(() => [])
  ...
  return (
    <>
      <input id={id} value={query} onChange={e => onQueryChange(e.target.value)}
        placeholder={status === "ready" && nearby.length ? "pick one below, or type a name" : "search a shop"}
      ...
      {isNew && (
        <p className="t-meta" style={{ fontSize: 13 }}>
          new shop · {place && (place.lat !== 0 || place.lng !== 0) ? "pinned where you are" : "no location yet"} — it’s added when you log this
        </p>
      )}

      {!picked && !q && status === "locating" && (
        <p className="t-meta" style={{ fontSize: 13 }}>finding boba near you…</p>
      )}
      ...
      {offerNew && (
        <div style={{ display: "flex", flexDirection: "column", gap: 8, paddingTop: 4 }}>
          <p className="t-meta" style={{ fontSize: 13, lineHeight: 1.5 }}>
            {suggestions.length === 0 ? `No shops called “${q}” yet.` : "Not the one you mean?"} Add it — you’ll be the first to log it.
          </p>
