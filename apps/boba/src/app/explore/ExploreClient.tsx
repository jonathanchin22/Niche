...
export default function ExploreClient({ places, firsts }: { places: LovedPlace[]; firsts: number }) {
  ...
  // Where you are, then every shop around it (the server seeds new areas).
  // Coming back from a shop shows the last list at once, then refreshes it.
  useEffect(() => {
    let cancelled = false
          ...
          {query.trim().length >= 2 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10, padding: "24px 0 8px" }}>
              <p className="t-meta" style={{ fontSize: 13, lineHeight: 1.5 }}>
                {results.length === 0 && mapResults.length === 0 ? `No shops called “${query.trim()}” yet.` : "Not seeing yours?"}{" "}
                Add it and log the first drink. It’ll be on boba! for everyone after, and it counts toward your firsts.
              </p>
              <Link href={`/log?cafe=${encodeURIComponent(query.trim())}`} className="btn btn-secondary" style={{ padding: "0 16px" }}>
                ＋ add “{query.trim()}” and log a drink
              </Link>
            </div>
          )}
        </section>
