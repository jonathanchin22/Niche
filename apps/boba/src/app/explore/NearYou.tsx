...
const CafeMap = dynamic(() => import("@/components/map/CafeMap"), {
  ssr: false,
  loading: () => <div style={{ height: 420, background: "var(--c-tint)", borderRadius: 18 }} />,
})

const KIND_LABEL: Record<string, string> = { specialty: "specialty", chain: "chain", casual: "shop" }
...
export default function NearYou({ here, status, places, firsts, area, searching, onSearchArea }: {
  ...
  // Come back to the same tab, view, pin and map position after opening a shop.
  useEffect(() => {
  ...
  // dozen chains shouldn't bury the one local shop.
  ...
  return (
    <section aria-labelledby="near-you">
      ...
      {tab === "first" && (
        <div style={{ margin: "0 24px 16px", padding: "16px 18px", border: "1px dashed var(--c-rule)", borderRadius: 18, display: "flex", flexDirection: "column", gap: 10 }}>
          <span className="t-title" style={{ fontSize: 20, lineHeight: 1.2 }}>
            {progress.count === 0 ? "Be the first to log a boba somewhere" : `You were first at ${progress.count} ${progress.count === 1 ? "shop" : "shops"}`}
          </span>
          <span className="t-meta" style={{ fontSize: 13, lineHeight: 1.5 }}>
            {progress.next
              ? `${progress.toNext} more ${progress.toNext === 1 ? "first" : "firsts"} for the ${progress.next.name} badge. The shop page will say you found it.`
      ...
      {(status === "locating" || status === "loading") && (
        <p className="t-meta" style={{ padding: "0 24px" }}>{status === "locating" ? "finding where you are…" : "finding boba around you…"}</p>
      )}
      {status === "no-location" && (
        <p className="t-meta" style={{ padding: "0 24px", lineHeight: 1.5 }}>Turn on location to see every boba shop around you, including ones nobody has logged yet.</p>
      )}
      {status === "error" && <p className="t-meta" style={{ padding: "0 24px" }}>Couldn’t load shops near you — try again in a bit.</p>}
      {status === "ready" && list.length === 0 && (
        <p className="t-meta" style={{ padding: "0 24px" }}>
          {tab === "first" ? "Every shop around here has been logged. Try the map further out." : "No boba shops found around here yet."}
        </p>
      )}
              ...
              <li key={p.id}>
                <Link href={`/place/${p.id}`} style={{ display: "grid", gridTemplateColumns: "52px 1fr auto", gap: 14, alignItems: "center", padding: "12px 0", borderBottom: "1px solid var(--c-rule)" }}>
                  <span aria-hidden="true" className={reviewed ? "t-score" : "t-display"} style={{
                    height: 52, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 18,
                    fontSize: reviewed ? 22 : 26, fontStyle: reviewed ? undefined : "italic",
                    color: reviewed ? "var(--c-jade)" : "var(--c-mid)",
                  ...
                  <span style={{ display: "flex", flexDirection: "column", gap: 3, minWidth: 0 }}>
                    ...
                    <span style={{ fontSize: 12, color: reviewed ? "var(--c-mid)" : "var(--c-ink)" }}>
                      {reviewed ? `${p.review_count} ${p.review_count === 1 ? "sip" : "sips"} logged` : "no sips yet · be the first"}
                    </span>
                  </span>
...
/** The shop you tapped on the map: name, what it is, its score or "be the first". */
function PreviewCard({ place, onClose }: { place: CatalogPlace; onClose: () => void }) {
  ...
  return (
    <div style={{ position: "absolute", left: 10, right: 10, bottom: 10, zIndex: 3, background: "var(--c-paper)", border: "1px solid var(--c-rule)", borderRadius: 18, boxShadow: "0 6px 18px rgba(28,20,16,.16)", display: "flex", alignItems: "stretch" }}>
      <Link href={`/place/${place.id}`} style={{ flexGrow: 1, minWidth: 0, display: "grid", gridTemplateColumns: "48px 1fr", gap: 12, alignItems: "center", padding: "12px 4px 12px 12px" }}>
        <span aria-hidden="true" className={reviewed ? "t-score" : "t-display"} style={{
          height: 48, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 18,
          fontSize: reviewed ? 20 : 24, fontStyle: reviewed ? undefined : "italic",
          color: reviewed ? "var(--c-jade)" : "var(--c-mid)", background: reviewed ? "var(--c-paper)" : "var(--c-tint)",
        ...
        <span style={{ display: "flex", flexDirection: "column", gap: 3, minWidth: 0 }}>
          ...
          <span style={{ fontSize: 12, color: "var(--c-ink)" }}>
            {reviewed ? `${place.review_count} ${place.review_count === 1 ? "sip" : "sips"} logged · open →` : "no sips yet · be the first →"}
          </span>
        </span>
      </Link>
