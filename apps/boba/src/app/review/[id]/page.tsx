...
export default async function ReviewPage({ params }: { params: { id: string } }) {
  ...
  // This drink was the first one logged at its shop.
  ...
  return (
    <AppShell nav={false}>
      ...
      <section style={{ display: "flex", flexDirection: "column", gap: 16, padding: "24px 24px 0" }}>
        ...
        {wasFirst && (
          <Link href={isOwn ? "/explore?tab=first" : `/place/${r.place!.id}`} style={{ alignSelf: "stretch", display: "flex", flexDirection: "column", gap: 4, padding: "12px 14px", border: "1px dashed var(--c-rule)", borderRadius: 18 }}>
            <span className="t-label" style={{ color: "var(--c-jade)" }}>✦ first sip ever logged here</span>
            {firsts && (
              <span className="t-meta" style={{ fontSize: 13, lineHeight: 1.5 }}>
                {`You've been first at ${firsts.count} ${firsts.count === 1 ? "shop" : "shops"}`}
