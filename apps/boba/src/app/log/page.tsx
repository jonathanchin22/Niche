              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 12 }}>
                {placeResults.map((p, i) => (
                  <button key={i} onClick={() => { setSelectedPlace(p); setPlaceQuery(p.name) }}
                    style={{ background: selectedPlace?.name === p.name ? "#e8f4ee" : "white", border: `1px solid ${selectedPlace?.name === p.name ? "#2d6a4f" : "#e8e8e4"}`, borderRadius: 8, padding: "12px 16px", textAlign: "left", cursor: "pointer" }}>
