...
export async function getUserReviews(
  ...
  const { data, error } = await supabase
    .from("reviews")
    .select("*, place:places!reviews_place_id_fkey(id, name, city, state)")
    .eq("user_id", user_id)
    .eq("app_id", app_id)
...
export async function getPlaceById(
  supabase: ReturnType<typeof createClient>,
  { place_id, app_id }: { place_id: string; app_id: string }
) {
  const { data, error } = await supabase
    .from("places")
    .select("*")
...
export async function getPlaceReviews(
  ...
  const { data, error } = await supabase
    .from("reviews")
    .select("*, profile:profiles!reviews_user_id_fkey(id, username, display_name)")
...
export async function getFollowing(
  ...
  const { data, error } = await supabase
    .from("follows")
    .select("following_id, profile:profiles!follows_following_id_fkey(id, username, display_name)")
    .eq("follower_id", user_id)
  if (error) throw error
  return data ?? []
}
