...
export function MyReviewsClient({ userId, initialReviews }: { userId: string; initialReviews: Review[] }) {
  ...
  const sorted = sortKey === "score"
    ...
    : [...filtered].sort((a, b) => {
        ...
        if (typeof va === "string") return sortDir === "desc" ? vb.localeCompare(va as string) : (va as string).localeCompare(vb as string)
  ...
  const handleDrop = useCallback((e: React.DragEvent, dropIndex: number) => {
    ...
    for (let i = 0; i < updated.length; i++) {
      ...
      if (updated[i].score > above.score) {
        // Can't be higher than item above — clamp to item above's score
        updated[i] = { ...updated[i], score: above.score }
      }
    }
